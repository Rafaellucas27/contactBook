...
function createCard(object){
    ...
    btnRemove.addEventListener('click', function(){
        ...
        console.log(`Excluiu ${object.name}`)
    })
...
function searchContact(){
  ...
  form.addEventListener('submit', function(event){
    ...
    for(let i = 0; i < contactsList.length; i++){
      // if(contactsList[i].name == searchInput.value.toLowerCase()){
      //   searchArray.push(contactsList[i]);
      // }
      if(contactsList[i].name.includes(searchInput.value.toLowerCase())){
          searchArray.push(contactsList[i])
      }
    }
